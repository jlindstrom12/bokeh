...
export type WhiskerData = GlyphData & p.UniformsOf<Whisker.Mixins> & {
  readonly _upper: Arrayable<number>
  readonly _lower: Arrayable<number>
  readonly _base: Arrayable<number>

  readonly slower: Arrayable<number>
  readonly supper: Arrayable<number>
  readonly sbase: Arrayable<number>

  lower_sx: Arrayable<number>
  lower_sy: Arrayable<number>
  upper_sx: Arrayable<number>
  upper_sy: Arrayable<number>
}

export interface WhiskerView extends WhiskerData {}
...
export class WhiskerView extends GlyphView {
  ...
  protected _index_data(index: SpatialIndex): void {
    ...
    const lower = [this._lower, this._base]
    const upper = [this._upper, this._base]
  ...
  override _map_data(): void {
    ...
    this.lower_sx = slower[i]
    this.lower_sy = slower[j]

    this.upper_sx = supper[i]
    this.upper_sy = supper[j]
  }

  protected _render(ctx: Context2d, indices: number[], data?: WhiskerData): void {
    const {lower_sx, upper_sx, lower_sy, upper_sy} = data ?? this
    const {lower_head, upper_head} = this
    const {dimension} = this.model
